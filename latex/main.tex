...
\section{Part I: PRNG and Random Variables} \label{part1}
  ...
  In NS-3 they generate normal distributed values using a rejection method called Box-Muller. In NS-3 they generate normal random values by generating two uniform random values u1 and u2 on the range [0,1] and then:
  $$v1 = 2 * u1 - 1$$
  $$v2 = 2 * u2 - 1$$
  $$w = v1^2 + v2^2$$
...
have to be careful choosing the attributes to the generator so that we dont get cycles early and avoid the zero-problem, 
...
Box-Muller. Finally we compared our exponetial random generator to the one in ns-3. Our implementation gave a steeper
...
Then when rebuilding ns3 our classes appeared as regular application classes.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{ns3_gs_queue_size.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{our_gs_queue_size.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{ns3_as_delay.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{ns3_sa_delay.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{our_as_delay.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{our_sa_delay.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{ns3_fg_delay.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\linewidth]{our_fg_delay.png}
...
This was mainly because we used git as a version control tool and in the beginning we thought that our own class implementation files had to be put into the ns3 source directory for it to be compiled.
Eventually we found that by modifying some of the building script we could put the class files in our project directory.
For scheduling the way we decided to implement it was good since we planed on replacing the random variables from ns3's implementation to our own implementation.
